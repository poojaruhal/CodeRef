...
df_train = pd.read_csv('../dataset/fine-tuning/new_large/code&comment-to-code/train.tsv', sep='\t', header=None)
...
for i in range(len(df)):
	pair = df.iloc[i]
	result = re.search('<code>(.*)</code>', pair[0]).group(1)
	output = code_vocab.encode_sequence(pair[1])
	encoded = code_vocab.encode_sequence(result)
	if len(encoded[0]) <= 60 and len(output[0]) <= 60:
		comment = re.search('<technical_language>(.*)</technical_language>', pair[0]).group(1)
		comment_length = len(nl_vocab.encode_sequence(comment)[0])
		if comment_length > l:
			l = comment_length
